...
public class GeneratorFileData {
	...
	// cancelled.
	public static GeneratorFileData create(final World world, final Player notifyPlayer) {
		final GeneratorFileData newData = new GeneratorFileData(world, notifyPlayer);
		...
		if (!newData.regionFolder.exists() || !newData.regionFolder.isDirectory()) {
			// check for region folder inside a DIM* folder (DIM-1 for nether, DIM1 for end,
			// DIMwhatever for custom world types)
			final File[] possibleDimFolders = newData.world.getWorldFolder().listFiles(new DimFolderFileFilter());
			for (final File possibleDimFolder : possibleDimFolders) {
				final File possible = new File(newData.world.getWorldFolder(),
		...
		// Accepted region file formats: MCR is from late beta versions through 1.1, MCA
		// is from 1.2+
	...
	// the constructor is private; use create() method above to create an instance
	// of this class.
	private GeneratorFileData(final World world, final Player notifyPlayer) {
		this.world = world;
	...
	public boolean doesChunkExist(final int x, final int z) {
		if(regionFiles == null || regionFolder == null)return true;
	...
	public boolean isChunkFullyGenerated(final int x, final int z) { // if all adjacent chunks exist, it should be a safe enough bet
		// that this one is fully generated
		return !(!doesChunkExist(x, z) || !doesChunkExist(x + 1, z) || !doesChunkExist(x - 1, z)
				|| !doesChunkExist(x, z + 1) || !doesChunkExist(x, z - 1));
	}
	...
	public void chunkExistsNow(final int x, final int z) {
		if(regionFiles == null || regionFolder == null)return;
		final CoordXZ region = new CoordXZ(CoordXZ.chunkToRegion(x), CoordXZ.chunkToRegion(z));
		final List<Boolean> regionChunks = this.getRegionData(region);
	...
	private int coordToRegionOffset(int x, int z) {
		// "%" modulus is used to convert potential world coordinates to definitely be
	...
	private List<Boolean> getRegionData(final CoordXZ region) {
		...
		// data for the specified region isn't loaded yet, so init it as empty and try
		// to find the file and load the data
		...
		for (int i = 0; i < regionFiles.length; i++) {
			final CoordXZ coord = regionFileCoordinates(i);
			...
			try {
				...
				// first 4096 bytes of region file consists of 4-byte int pointers to chunk data
				// in the file (32*32 chunks = 1024; 1024 chunks * 4 bytes each = 4096)
				...
				sendMessage(
						"Error! Could not open region file to find generated chunks: " + this.regionFile(i).getName());
			} catch (final IOException ex) {
				sendMessage(
						"Error! Could not read region file to find generated chunks: " + this.regionFile(i).getName());
			}
		}
	...
	private void sendMessage(final String text) {
		GeneratorManager.log("[WorldData] " + text);
